...
transformed parameters{
  ...
  for(n in 1:NRegions){
    for(t in 1:MAxYear){
      log_N[t,n] = alpha[n] + 
                   beta1[n]*Ponds[t] +
                   beta2[n]*PPRprec[t] + beta3[n]*Borealprec[t] + 
                   beta4[n]*PPRmaxt[t] + beta5[n]*Borealmaxt[t] + epsilon_N[t,n];
  }}
}
model{
...
generated quantities{
  ...
  for(n in 1:NRegions){
     pred_log_N[n] =  alpha[n] + 
                      beta1[n]*Ponds[(MAxYear+1)] +
                      beta2[n]*PPRprec[(MAxYear+1)] + beta3[n]*Borealprec[(MAxYear+1)] + 
                      beta4[n]*PPRmaxt[(MAxYear+1)] + beta5[n]*Borealmaxt[(MAxYear+1)] +  epsilon_pred[n];
  }
