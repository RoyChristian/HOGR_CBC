...
data{
  ...
  vector[NYear] PPRprec;
  vector[NYear] Borealprec;
  vector[NYear] PPRmaxt;
  vector[NYear] Borealmaxt;
  matrix[NYear,NRegions] Wprec;
  matrix[NYear,NRegions] Wmint;
  vector[NYear] Trend;
  //Distance matrix
...
transformed parameters{
  ...
  vector[NRegions] beta2;
  vector[NRegions] beta3;
  vector[NRegions] beta4;
  vector[NRegions] beta5;
  vector[NRegions] beta6;
  vector[NRegions] beta7;
  //Population
  ...
  beta2 = mu_param[3] + to_vector(param_epsilon[,3]);
  beta3  = mu_param[4] + to_vector(param_epsilon[,4]);
  beta4  = mu_param[5] + to_vector(param_epsilon[,5]);
  beta5  = mu_param[6] + to_vector(param_epsilon[,6]);
  beta6  = mu_param[7] + to_vector(param_epsilon[,7]);
  beta7  = mu_param[8] + to_vector(param_epsilon[,8]);
  ...
  for(n in 1:NRegions){
    for(t in 1:MAxYear){
      log_N[t,n] = alpha[n] + 
                   beta1[n]*PPRprec[t] + beta2[n]*Borealprec[t] + 
                   beta3[n]*PPRmaxt[t] + beta4[n]*Borealmaxt[t] + 
                   beta5[n]*Wprec[t,n] + beta6[n]*Wmint[t,n] + 
                   beta7[n]*Trend[t] + epsilon_N[t,n];
  }}
}
model{
...
generated quantities{
  ...
  for(n in 1:NRegions){
     pred_log_N[n] =  alpha[n] + 
                      beta1[n]*PPRprec[(MAxYear+1)] + beta2[n]*Borealprec[(MAxYear+1)] + 
                      beta3[n]*PPRmaxt[(MAxYear+1)] + beta4[n]*Borealmaxt[(MAxYear+1)] +  
                      beta5[n]*Wprec[(MAxYear+1),n] + beta6[n]*Wmint[(MAxYear+1),n] +
                      beta7[n]*Trend[(MAxYear+1)] + epsilon_pred[n];
  }
