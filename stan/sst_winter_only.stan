...
transformed parameters{
  ...
  for(n in 1:NRegions){
    for(t in 1:MAxYear){
      log_N[t,n] = alpha[n] + beta1[n]*Wprec[t,n] + beta2[n]*Wmint[t,n] + epsilon_N[t,n];
  }}
}
model{
...
generated quantities{
  ...
  for(n in 1:NRegions){
     pred_log_N[n] =  alpha[n] + beta1[n]*Wprec[(MAxYear+1),n] + beta2[n]*Wmint[(MAxYear+1),n] + epsilon_pred[n];
  }
