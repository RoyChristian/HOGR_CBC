...
# Filter for the 1970-2017 study period
ponds.df <- ponds.df %>% filter(Year %in% seq(1970, 2017))
...
################################################################################
# Using st_read for a more modern sf-based workflow
...
new.order    <- order(distance.mat[7, ]) # Anchored to the 7th stratum (Alaska)
...
data.stan <- list(NYear = nrow(mean.ordered), 
                  ...
                  Ponds = scale(ponds.df$Ponds / 1e6, scale = FALSE)[, 1],
...
#                                 run Stan model
################################################################################
full.model <- cmdstan_model(stan_file = file.path(root, "stan", "sst_breeding_all.stan"))
...
fit <- full.model$sample(data = data.stan,
                         ...
                         init = function() {
                           ...
                           # Explicitly matching MAxYear for cross-validation stability
...
save.image(file.path(root, paste0("output/full_breeding_all_", n_years_modeled, ".RData")))
fit$save_object(file = file.path(root, paste0("output/fit_full_breeding_all_N", n_years_modeled, "fit.RDS")))
...
write.csv(pred.df, file = file.path(root, paste0("output/fit_breeding_all_N", n_years_modeled, "lppd.csv")), row.names = FALSE)