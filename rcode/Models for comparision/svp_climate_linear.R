...
#### Join ponds and summer climate
ponds.df = left_join(ponds.df, breed.df)

################################################################################
#                           import shapefiles (sf)
################################################################################
# Using the st_read approach consistent with previous scripts
...
#                         run Stan model
################################################################################
full.model <- cmdstan_model(stan_file=file.path(root,"stan","sst_climate_linear.stan"))
...
#                         save output
################################################################################
save.image(file.path(root, paste0("output/climate_linear_", (nrow(mean.ordered)-Kept_out), ".RData")))
fit$save_object(file = file.path(root, paste0("output/fit_climate_linear_N", (nrow(mean.ordered)-Kept_out), "fit.RDS")))

################################################################################
#                         diagnose and summary
################################################################################
...
write.csv(pred.df, file = file.path(root, paste0("output/fit_climate_linear_N", (nrow(mean.ordered)-Kept_out), "lppd.csv")), row.names = FALSE)