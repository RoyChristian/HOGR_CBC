...
### Keep only 1970 to 2017 and create lag
...
data.stan <- list(NYear = nrow(mean.ordered), 
                  ...
                  # Scaling ponds by 1 million for better convergence
                  Ponds = scale(ponds.df$Ponds/1e6, scale = FALSE)[,1])

################################################################################
#                                 run Stan model
################################################################################
full.model <- cmdstan_model(stan_file=file.path(root,"stan","sst_breeding_ponds_only.stan"))
...
save.image(file.path(root, paste0("output/breeding_ponds_only_", n_years_modeled, ".RData")))
fit$save_object(file = file.path(root, paste0("output/fit_breeding_ponds_only_N", n_years_modeled, "fit.RDS")))

################################################################################
#                                  diagnose
################################################################################
fit_summary <- fit$summary()
# Using the stricter 1.05 threshold for publication-ready models
...
write.csv(pred.df, file = file.path(root, paste0("output/fit_breeding_ponds_only_N", n_years_modeled, "lppd.csv")), row.names = FALSE)