...
# Filtering for the study period
...
### Join winter and breeding climate
hogr.df <- left_join(hogr.df, winter.df, by = c("stratum2", "year"))
# Join breeding conditions to the main index data
hogr.df <- left_join(hogr.df, breed.df, by = c("year" = "Year"))
...
new.order    <- order(distance.mat[7, ]) # Distance from Alaska reference
...
#                                 run Stan model
################################################################################
full.model <- cmdstan_model(stan_file = file.path(root, "stan", "sst_breeding_noponds_winter.stan"))
...
save.image(file.path(root, paste0("output/breeding_noponds_winter_", n_years_modeled, ".RData")))
fit$save_object(file = file.path(root, paste0("output/fit_breeding_noponds_winter_N", n_years_modeled, "fit.RDS")))
...
write.csv(pred.df, file = file.path(root, paste0("output/fit_breeding_noponds_winter_N", n_years_modeled, "lppd.csv")), row.names = FALSE)